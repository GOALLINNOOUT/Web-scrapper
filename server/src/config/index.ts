...
export const config = {
  ...
  goCrawlerEnabled: process.env.GO_CRAWLER_ENABLED === 'true',
  goCrawlerCommand: process.env.GO_CRAWLER_COMMAND || '',
  goCrawlerMaxActiveCrawls: clamp(process.env.MAX_ACTIVE_CRAWLS, 1, 3, 2),
  goCrawlerFetchConcurrency: clamp(process.env.GO_FETCH_CONCURRENCY, 1, 10, 5),
  goCrawlerRuntimeMinutes: clamp(process.env.MAX_RUNTIME_PER_CRAWL_MINUTES, 1, 240, 20),
  goCrawlerIdleTimeoutMs: clamp(process.env.GO_CRAWLER_IDLE_TIMEOUT_MS, 10_000, 600_000, 180_000),
  goCrawlerNoPageCap: process.env.GO_CRAWLER_NO_PAGE_CAP !== 'false',
  pageHtmlArchiveEnabled: process.env.PAGE_HTML_ARCHIVE_ENABLED !== 'false',
  pageHtmlArchiveMaxBytes: clamp(process.env.PAGE_HTML_ARCHIVE_MAX_BYTES, 0, 10 * 1024 * 1024, 2 * 1024 * 1024),
  ...
  cacheTtlMonitoringMs: clamp(process.env.CACHE_TTL_MONITORING_MS, 0, 60_000, 3000),
  cacheTtlCrawlMs: clamp(process.env.CACHE_TTL_CRAWL_MS, 0, 60_000, 3000),
  cacheTtlDataMs: clamp(process.env.CACHE_TTL_DATA_MS, 0, 300_000, 10_000),
  cacheTtlDomainMs: clamp(process.env.CACHE_TTL_DOMAIN_MS, 0, 300_000, 15_000),
