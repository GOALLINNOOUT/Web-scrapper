...
export async function evaluateAlerts() {
  const [metric, queue, domain] = await Promise.all([
    MetricSnapshot.findOne().sort({ timestamp: -1 }).lean(),
    QueueMetric.findOne().sort({ timestamp: -1 }).lean(),
    DomainMetric.findOne({ sample_count: { $gt: 0 }, block_rate_percent: { $ne: null } }).sort({ block_rate_percent: -1, timestamp: -1 }).lean()
  ]);
  if (!metric) return [];

  const candidates = await buildCandidates(metric as MetricDoc, queue as QueueDoc | null, domain as DomainDoc | null);
...
async function buildCandidates(metric: MetricDoc, queue: QueueDoc | null, domain: DomainDoc | null): Promise<CandidateAlert[]> {
  const candidates: CandidateAlert[] = [];
  pushThreshold(candidates, {
    key: 'workers:cpu_average',
    component: 'workers',
    title: 'Worker CPU saturation',
    description: 'Average worker CPU is above the configured threshold.',
    suggestion: metric.workers.avg_cpu > 90 && (queue?.backlog_growth_rate_per_min || 0) > 0
      ...
      : 'Inspect worker concurrency and slow crawl targets.',
  ...
  pushThreshold(candidates, {
    ...
    description: 'Backlog growth indicates crawler throughput is below incoming workload.',
    suggestion: 'Increase crawl worker capacity or reduce enqueue rate until the backlog drains.',
  ...
  pushThreshold(candidates, {
    ...
    description: 'Redis memory usage is approaching the configured maxmemory limit.',
    suggestion: 'Review queue retention and Redis maxmemory settings.',
    value: redisPercent,
  ...
  pushThreshold(candidates, {
    ...
    description: 'MongoDB P95 latency is above the operational threshold.',
    suggestion: 'Check for missing indexes or lock contention on recent queries.',
  ...
  pushInverseThreshold(candidates, {
    ...
    description: 'Proxy success rate is below the configured threshold.',
    suggestion: 'Proxy provider degradation or target site block pattern change.',
  ...
  if (domain) {
    pushThreshold(candidates, {
      ...
      description: 'A target domain is returning elevated block/captcha indicators.',
      suggestion: 'Reduce crawl rate for this domain and rotate request identity.',
  ...
  pushThreshold(candidates, {
    ...
    description: 'Recent failure volume is elevated compared with the 15 minute average.',
    suggestion: 'Open the failures page and inspect dominant failure type and affected domains.',
...
async function resolveStaleActiveAlerts(seen: Set<string>) {
  const alerts = await AlertRecord.find({ status: 'active' }).sort({ timestamp: -1 }).select('alert_id alert_key timestamp status resolved_at duration_seconds');
