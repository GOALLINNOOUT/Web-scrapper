    ...
    <div className="mobile-page-enter mx-auto w-full max-w-[400px] px-4 pb-[112px] pt-2">
      ...
      <div className="grid gap-2">
        {!loadError && jobs.map((job) => {
          ...
          return (
            <button className="mobile-crawl-card mobile-tap w-full min-w-0 overflow-hidden text-left" key={job._id} type="button" onClick={() => navigate(`/crawls/${job._id}`)}>
              ...
              <div className="mt-3 grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
                <span className="flex min-w-0 gap-2 truncate text-[11px] font-semibold text-[var(--text-secondary)]"><Mail className="shrink-0" size={13} /> {job.emailsFound}<Share2 className="shrink-0" size={13} /> {job.socialLinksFound}</span>
