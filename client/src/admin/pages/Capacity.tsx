  ...
  return (
    <div className="admin-page">
      ...
      {capacity.loading && !capacity.data ? <SkeletonGrid count={4} /> : <section className="admin-grid-four">
        {(capacity.data || []).map((item) => <MetricCard key={item.metric} title={item.metric} value={item.days_remaining ?? 'No date'} unit={item.days_remaining ? 'days' : ''} trend={(item.days_remaining || 999) < 30 ? 'up' : 'stable'} trendValue={`${formatNumber(item.growth_rate_per_day)} / day`} status={(item.days_remaining || 999) < 14 ? 'critical' : 'warning'} />)}
      </section>}
      ...
      {costs.loading && !costs.data ? <SkeletonGrid count={4} /> : <section className="admin-grid-four">
        <MetricCard title="Daily Cost" value={formatNumber(latestCost.total_daily_usd)} unit="USD" />
        <MetricCard title="Weekly Cost" value={formatNumber(latestCost.total_weekly_usd)} unit="USD" />
        <MetricCard title="Monthly Projected" value={formatNumber(latestCost.total_monthly_usd_projected)} unit="USD" />
        <MetricCard title="Cost Rows" value={costRows.length} />
      </section>}
